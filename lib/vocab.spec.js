...
describe("Returns names nicely ", async function() {
  ...
  it("Should find all my vocab", async function() {
     ...
     assert(an["@id"] === "https://w3id.org/ldac/terms#Annotation");
     assert.equal(v.count("DefinedTerm"), 46);
     assert.equal(v.count("DefinedTermSet"), 10);
     assert.equal(v.count("rdf:Property"), 54);
     assert.equal(v.count("rdfs:Class"), 5);
    const cont = v.getContext();
    assert.equal(cont["Annotation"], "https://w3id.org/ldac/terms#Annotation");
